...
async fn handle_resolve_response(
    services: Services,
    packet: dns_parser::Packet<'_>,
    addr: SocketAddr,
) -> Option<Vec<u8>> {
    if packet.header.truncated {
        warn!(
            "handle_resolve_response : dropping truncated packet from {:?}",
            addr
        );
        return None;
    }
...
async fn handle_resolve_request(
    ...
    if packet.header.truncated {
        warn!(
            "handle_resolve_request : dropping truncated packet from {:?}",
            addr
        );
        return None;
    }
    ...
    let mut unicast_builder =
        dns_parser::Builder::new_response(packet.header.id, false).move_to::<dns_parser::Answers>();
    ...
    for question in packet.questions {
        ...
        if question.qclass == QueryClass::IN || question.qclass == QueryClass::Any {
            if question.qu {
                unicast_builder =
                    handle_question(af_v6, &services, &question, unicast_builder).await;
                debug!("Unicast builder selected")
            } else {
                multicast_builder =
                    handle_question(af_v6, &services, &question, multicast_builder).await;
    ...
    loop {
        ...
        match res {
            Ok(Ok((bytes, addr))) => {
                if bytes > buf.len() {
                    warn!(
                        "Discarding as packet with {} bytes is too long for the buffer {}",
                        bytes,
                        buf.len()
                    );
                };
                ...
                match dns_parser::Packet::parse(&buf[..bytes]) {
                    Ok(packet) => {
                        debug!(
                            "received packet from {:?} with header {:?} ",
                            addr, packet.header
                        );
                        if packet.header.query {
                            let res =
                                handle_resolve_request(af_v6, advertised_services, packet, addr)
                                    .await;
                            if let Some(response) = res {
                                let res = outgoing
                                    .send(Command::SendResponse { response, addr })
                                    .await;
                                if let Err(e) = res {
                                    info!("Can't send {:?}", e);
                                    return;
                            ...
                            let _res =
                                handle_resolve_response(resolved_services, packet, addr).await;
                        }
                    }
                    Err(error) => {
...
fn prepare_unsolicited(
    af_v6: bool,
    mdns_group: IpAddr,
    hostname: &dns_parser::Name<'_>,
    svc: &ServiceData,
    ttl: u32,
    include_ip: bool,
    ...
    if !builder.is_empty() {
        Some((
            ...
            SocketAddr::new(mdns_group, MDNS_PORT),
        ))
    } else {
        None
    }
    ...
    while let Some(command) = commands.recv().await {
        ...
        match command {
            ...
            Command::SendUnsolicited {
                ...
                if let Some((response, addr)) =
                    prepare_unsolicited(af_v6, mdns_group, hostname, &svc, ttl, include_ip)
                {
            ...
            Command::SendResolveRequest { svc } => {
